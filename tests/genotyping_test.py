...
class MultiallelicVCFGenotypeTest(unittest.TestCase):
    ...
    def test_genotype_multiallelic(self, mock_model):
        ...
        instance.predict.side_effect = [
            (None, tf.constant([[0.5, 0.5, 0.9]])),
            (None, tf.constant([[0.51, 0.51, 0.92]])),
            (None, tf.constant([[0.52, 0.52, 0.12]])),
        ]
        ...
        np.testing.assert_array_almost_equal(record.samples["HG002"]["DHFFC"], (0.27937498688697815, 0.3624579906463623), decimal=3)
        np.testing.assert_array_equal(record.samples["HG002"]["FS"], [0, 0])
        np.testing.assert_array_almost_equal(record.samples["HG002"]["SOR"], (0.693, 0.693), decimal=3)
